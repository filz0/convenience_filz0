...
==================================================================================================
                    MODULE BY (TTT2)
					https://github.com/WardenPotato
					https://github.com/ShadowBonnieRUS
==================================================================================================
--]]
...
module( "conv_outline", package.seeall )
...
local outline_mat = CreateMaterial("conv_outline", "UnlitGeneric", outline_mat_settings)
...
-- @realm client
function Add(ents, color, mode, render_type, outline_thickness, include_children)
...
local function RenderModels(ents, children)
...
local function Render()
    ...
    for i = 1, ListSize do
        ...
        local render_children = data[CHILDREN]
...
local function CONVRenderOutlines()
    -- @realm client
    hook.Run("CONVSetupOutlines", GetRenderType())
...
hook.Add("PreDrawViewModels", "CONVRenderOutlines", function()
    SetRenderType(OUTLINE_RENDERTYPE_BEFORE_VM)
    CONVRenderOutlines()
end)

hook.Add("PreDrawEffects", "CONVRenderOutlines", function()
    SetRenderType(OUTLINE_RENDERTYPE_BEFORE_EF)
    CONVRenderOutlines()
end)

hook.Add("PostDrawEffects", "CONVRenderOutlines", function()
